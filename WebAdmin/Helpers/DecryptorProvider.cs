...
{
    public interface IDecryptorProvider : IDisposable
    {
        string Decrypt(string a);
        string Encrypt(string a);
    }
    public class DecryptorProvider : IDecryptorProvider
    {
        const string passPhrase = "WebAdmin-BacNgocTuan";
        const string initVector = "@1B2c3D4e5F6g7H8";
        //private static readonly IHttpContextAccessor _httpContextAccessor;
        //public static HttpContext httpContext => _httpContextAccessor.HttpContext;
        private readonly ILogger<DecryptorProvider> _logger;
        private RijndaelEnhanced _rijndael;
        public DecryptorProvider(ILogger<DecryptorProvider> _logger)
        {
            this._logger = _logger;
            _rijndael = new RijndaelEnhanced(passPhrase, initVector);
        }
        public string Decrypt(string a)
        {
            //var _logger = httpContext.RequestServices.GetService(typeof(ILogger)) as Logger;
            string s = "";
            if (!(a == null || a == ""))
            {
                try
                {
                    s = _rijndael.Decrypt(a);
                    //_logger.LogInformation($"Giai ma {a}: {s}");
                }
                catch (Exception ex)
                {
                    s = a;
                    _logger.LogError($"Giai ma {a}: {ex.ToString()}");
                }
            }
        ...
        public string Encrypt(string a)
        {
            ...
            if (!(a == null || a == ""))
            {
                try
                {
                    s = _rijndael.Encrypt(a);
                    //_logger.LogInformation($"Ma hoa {a}: {s}");
                }
                catch (Exception ex)
                {
                    s = a;
                    _logger.LogError($"Ma hoa {a}: {ex.ToString()}");
                }
            }
