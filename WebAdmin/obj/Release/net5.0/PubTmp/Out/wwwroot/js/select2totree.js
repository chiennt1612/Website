...
(function ($) {
	$.fn.select2ToTree = function (options) {
		...
		opts.templateResult = function (data, container) {
			...
			var $iteme = $("<span class='item-label' data-toggle='tooltip' title='"+ label +"'  data-placement='left' ></span>").append(label);
			if (data.element) {
...
// this.attr('size',"10");
		...
		s2inst.on("select2:open", function (evt) {
			...
			$(document.getElementsByClassName('select2-dropdown--below')).css('width', $(document.getElementsByClassName('select2-results__options')).get(0).scrollWidth + 50);
			$(document.getElementsByClassName('select2-results__options')).css('max-height', window.innerHeight - ($(document.getElementsByClassName('select2-results__options')).offset().top + 26));
		});
