...
namespace SSO.Controllers.Account
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (button != "login")
            {
                ...
                else if (button == "register")
                {
                    ...
                    return RedirectToAction(nameof(Register), new { model.ReturnUrl });
                }
                else if (button == "forgotpassword")
                {
                    // since we don't have a valid context, then we just go back to the home page
                    return RedirectToAction(nameof(ForgotPassword), new { model.ReturnUrl });
                }
                else
                {
                    // since we don't have a valid context, then we just go back to the home page
        ...
        public async Task<IActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            var code = "";
            switch (model.SelectedProvider.ToLower())
            {
                case "phone":
                    code = await _userManager.GenerateTwoFactorTokenAsync(user, TokenOptions.DefaultPhoneProvider);
                    ...
                    break;
                case "email":
                    code = await _userManager.GenerateTwoFactorTokenAsync(user, TokenOptions.DefaultEmailProvider);
                    if (string.IsNullOrWhiteSpace(code))
                    {
                        return View("Error");
                    }
                ...
                default: // "authenticator"
                    code = await _userManager.GenerateTwoFactorTokenAsync(user, TokenOptions.DefaultAuthenticatorProvider);
                    break;
            }
